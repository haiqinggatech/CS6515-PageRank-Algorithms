    ...
    def solveRankIter(self, oldRankVec):
        ...
        if self.selfLoops == 1:
            ...
            for item in self.in_list:
                summation = 0
                for node in item: 
                    summation += (oldRankVec[node] / self.out_degree[node])

                temp_list.append(const +summation *self.alpha)


        else:
            summation = 0
            for item in self.sink_nodes:
                summation += oldRankVec[item]
            const = (( self.alpha * summation) + (1 - self.alpha)) / self.N
            #     summation = (oldRankVec[item] + summation)
            # const +=  summation*self.alpha / self.N
            temp_list = []    
...
def main():	
    ...
    parser.add_argument('-s', '--selfloop', help='Use Self Loops to handle sinks.', action='store_true', default=True, dest='selfLoops')
